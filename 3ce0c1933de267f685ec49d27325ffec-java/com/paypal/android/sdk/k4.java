package com.paypal.android.sdk;

import android.content.Context;
import android.util.TypedValue;
import android.view.ViewGroup;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.TextView;
/* loaded from: classes2.dex */
public final class k4 {

    /* renamed from: a  reason: collision with root package name */
    public ViewGroup f21409a;

    /* renamed from: b  reason: collision with root package name */
    public Button f21410b;

    /* renamed from: c  reason: collision with root package name */
    public p4 f21411c;

    /* renamed from: d  reason: collision with root package name */
    public TextView f21412d;

    /* renamed from: e  reason: collision with root package name */
    private LinearLayout f21413e;

    /* renamed from: f  reason: collision with root package name */
    public TextView f21414f;

    /* renamed from: g  reason: collision with root package name */
    public TextView f21415g;

    /* renamed from: h  reason: collision with root package name */
    public ImageView f21416h;

    /* renamed from: i  reason: collision with root package name */
    public LinearLayout f21417i;

    /* renamed from: j  reason: collision with root package name */
    public c4 f21418j;

    /* renamed from: k  reason: collision with root package name */
    public LinearLayout f21419k;

    /* renamed from: l  reason: collision with root package name */
    public Button f21420l;

    /* renamed from: m  reason: collision with root package name */
    public TextView f21421m;

    /* renamed from: n  reason: collision with root package name */
    public ImageView f21422n;

    public k4(Context context) {
        ViewGroup e10 = u1.e(context);
        this.f21409a = e10;
        LinearLayout h10 = u1.h(e10);
        this.f21413e = h10;
        LinearLayout w10 = u1.w(h10);
        TextView textView = new TextView(context);
        this.f21421m = textView;
        textView.setText("Your Order");
        u1.q(this.f21421m, "0dip", "0dip", "0dip", "14dip");
        this.f21421m.setTextSize(24.0f);
        this.f21421m.setTextColor(t1.f21978e);
        w10.addView(this.f21421m, new ViewGroup.LayoutParams(-2, -2));
        p4 p4Var = new p4(context, "$0.00");
        this.f21411c = p4Var;
        w10.addView(p4Var.f21507b);
        u1.p(this.f21411c.f21507b, null, "16dip");
        this.f21419k = u1.g(context, true, 42044, w10);
        ImageView imageView = new ImageView(context);
        imageView.setImageBitmap(u1.B("", context));
        imageView.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
        imageView.setAdjustViewBounds(true);
        imageView.setContentDescription("PayPal");
        if (z3.f22069a) {
            this.f21419k.addView(imageView);
            u1.y(imageView, "4dip", "2.5dip", null, null);
            u1.n(imageView, -2, "28dip");
        }
        TextView textView2 = new TextView(context);
        this.f21415g = textView2;
        u1.t(textView2);
        this.f21415g.setText("Pay with");
        this.f21419k.addView(this.f21415g);
        if (!z3.f22069a) {
            this.f21419k.addView(imageView);
            u1.y(imageView, "4dip", "2.5dip", null, null);
            u1.n(imageView, -2, "28dip");
        }
        LinearLayout linearLayout = new LinearLayout(context);
        linearLayout.setOrientation(1);
        w10.addView(linearLayout, new LinearLayout.LayoutParams(-1, -2));
        u1.y(linearLayout, null, null, null, "16dip");
        TextView textView3 = new TextView(context);
        this.f21412d = textView3;
        textView3.setTextSize(14.0f);
        this.f21412d.setTextColor(t1.f21998y);
        this.f21412d.setGravity(17);
        linearLayout.addView(this.f21412d, new LinearLayout.LayoutParams(-1, -2));
        u1.y(this.f21412d, null, "8dip", null, "4dip");
        Button button = new Button(context);
        this.f21410b = button;
        linearLayout.addView(button);
        u1.s(this.f21410b);
        u1.n(this.f21410b, -2, "24dip");
        u1.l(this.f21410b, 17, 1.0f);
        this.f21417i = u1.g(context, false, 42043, w10);
        ImageView imageView2 = new ImageView(context);
        this.f21422n = imageView2;
        imageView2.setAdjustViewBounds(true);
        this.f21422n.setContentDescription("PayPal");
        this.f21422n.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
        this.f21422n.setImageBitmap(u1.B("", context));
        this.f21417i.addView(this.f21422n, new ViewGroup.LayoutParams(-2, (int) TypedValue.applyDimension(1, 22.0f, context.getResources().getDisplayMetrics())));
        u1.y(this.f21422n, null, null, "6dip", "1dip");
        u1.n(this.f21422n, -2, "22dip");
        this.f21422n.setVisibility(8);
        ImageView imageView3 = new ImageView(context);
        this.f21416h = imageView3;
        imageView3.setAdjustViewBounds(true);
        this.f21416h.setContentDescription("PayPal");
        this.f21416h.setScaleType(ImageView.ScaleType.CENTER_INSIDE);
        this.f21416h.setVisibility(8);
        this.f21417i.addView(this.f21416h, new ViewGroup.LayoutParams(-2, (int) TypedValue.applyDimension(1, 22.0f, context.getResources().getDisplayMetrics())));
        u1.y(this.f21416h, null, null, "4dip", null);
        u1.x(this.f21416h, "36dip", "25dip");
        TextView textView4 = new TextView(context);
        this.f21414f = textView4;
        u1.t(textView4);
        this.f21414f.setText("Pay with Card");
        this.f21417i.addView(this.f21414f, new ViewGroup.LayoutParams(-2, -2));
        Button button2 = new Button(context);
        this.f21420l = button2;
        button2.setId(42042);
        u1.s(this.f21420l);
        w10.addView(this.f21420l);
        u1.y(this.f21420l, null, null, null, "16dip");
        u1.m(this.f21420l, -2, -2);
        u1.l(this.f21420l, 17, 1.0f);
        c4 c4Var = new c4(context);
        this.f21418j = c4Var;
        w10.addView(c4Var.f21085a);
        u1.m(this.f21418j.f21085a, -2, -2);
        u1.l(this.f21418j.f21085a, 17, 1.0f);
    }
}
