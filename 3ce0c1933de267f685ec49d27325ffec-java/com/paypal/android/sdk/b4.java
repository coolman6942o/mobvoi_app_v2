package com.paypal.android.sdk;

import android.content.Context;
import android.text.method.LinkMovementMethod;
import android.view.ViewGroup;
import android.widget.Button;
import android.widget.ImageView;
import android.widget.LinearLayout;
import android.widget.TextView;
/* loaded from: classes2.dex */
public final class b4 {

    /* renamed from: a  reason: collision with root package name */
    public ViewGroup f21016a;

    /* renamed from: b  reason: collision with root package name */
    private ViewGroup f21017b;

    /* renamed from: c  reason: collision with root package name */
    public TextView f21018c;

    /* renamed from: d  reason: collision with root package name */
    public TextView[] f21019d = new TextView[10];

    /* renamed from: e  reason: collision with root package name */
    public TextView f21020e;

    /* renamed from: f  reason: collision with root package name */
    public TextView f21021f;

    /* renamed from: g  reason: collision with root package name */
    public c4 f21022g;

    /* renamed from: h  reason: collision with root package name */
    public Button f21023h;

    /* renamed from: i  reason: collision with root package name */
    private LinearLayout f21024i;

    /* renamed from: j  reason: collision with root package name */
    public LinearLayout f21025j;

    /* renamed from: k  reason: collision with root package name */
    public TextView f21026k;

    /* renamed from: l  reason: collision with root package name */
    private ImageView f21027l;

    public b4(Context context) {
        ViewGroup e10 = u1.e(context);
        this.f21017b = e10;
        LinearLayout h10 = u1.h(e10);
        this.f21024i = h10;
        LinearLayout w10 = u1.w(h10);
        TextView textView = new TextView(context);
        this.f21018c = textView;
        u1.q(textView, "0dip", "0dip", "0dip", "14dip");
        this.f21018c.setTextSize(24.0f);
        this.f21018c.setTextColor(t1.f21978e);
        w10.addView(this.f21018c);
        u1.m(this.f21018c, -2, -2);
        ImageView imageView = new ImageView(context);
        this.f21027l = imageView;
        imageView.setImageBitmap(u1.B("", context));
        this.f21027l.setContentDescription("PayPal");
        w10.addView(this.f21027l);
        u1.n(this.f21027l, -1, "40dip");
        u1.y(this.f21027l, null, null, null, "10dip");
        for (int i10 = 0; i10 < 10; i10++) {
            TextView[] textViewArr = this.f21019d;
            TextView textView2 = new TextView(context);
            textView2.setTextColor(t1.f21997x);
            textView2.setLinkTextColor(t1.G);
            textView2.setTypeface(t1.F);
            textView2.setTextSize(18.0f);
            textView2.setSingleLine(false);
            textView2.setMovementMethod(LinkMovementMethod.getInstance());
            w10.addView(textView2);
            u1.m(textView2, -1, -2);
            u1.p(textView2, null, "10dip");
            textViewArr[i10] = textView2;
            this.f21019d[i10].setFocusable(false);
            this.f21019d[i10].setVisibility(8);
            this.f21019d[i10].setId(i10 + 100);
        }
        TextView textView3 = new TextView(context);
        this.f21020e = textView3;
        textView3.setId(2);
        u1.z(this.f21020e);
        w10.addView(this.f21020e);
        u1.m(this.f21020e, -1, -2);
        u1.p(this.f21020e, "20dip", "10dip");
        this.f21020e.setNextFocusRightId(1);
        TextView textView4 = new TextView(context);
        this.f21021f = textView4;
        textView4.setId(4);
        u1.z(this.f21021f);
        w10.addView(this.f21021f);
        u1.m(this.f21021f, -1, -2);
        u1.y(this.f21021f, null, "20dip", null, "10dip");
        this.f21021f.setVisibility(8);
        LinearLayout g10 = u1.g(context, true, 1, w10);
        this.f21025j = g10;
        g10.setNextFocusLeftId(2);
        this.f21025j.setNextFocusRightId(3);
        TextView textView5 = new TextView(context);
        this.f21026k = textView5;
        u1.t(textView5);
        this.f21026k.setText("Agree");
        this.f21025j.addView(this.f21026k, new ViewGroup.LayoutParams(-2, -2));
        Button button = new Button(context);
        this.f21023h = button;
        button.setId(3);
        u1.s(this.f21023h);
        w10.addView(this.f21023h);
        u1.y(this.f21023h, null, null, null, "16dip");
        u1.m(this.f21023h, -2, -2);
        u1.l(this.f21023h, 17, 1.0f);
        this.f21023h.setNextFocusLeftId(1);
        c4 c4Var = new c4(context);
        this.f21022g = c4Var;
        this.f21024i.addView(c4Var.f21085a);
        u1.m(this.f21022g.f21085a, -2, -2);
        u1.l(this.f21022g.f21085a, 17, 1.0f);
        this.f21016a = this.f21017b;
    }
}
