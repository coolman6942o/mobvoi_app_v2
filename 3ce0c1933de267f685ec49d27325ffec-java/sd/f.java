...
import mk.b;
import qd.d;
/* compiled from: TsmBluetoothCallback.java */
/* loaded from: classes2.dex */
public class f implements IMobvoiTsmBluetoothInterface {
    ...
    private List<Rapdu> a(d dVar, List<Capdu> list) {
        ArrayList arrayList = new ArrayList();
        list.size();
        for (Capdu capdu : list) {
            byte[] b10 = b.b(capdu.getApdu());
            k.c("TsmBluetoothCallback", "capdu index: %s, apdu %s, exp sw %s", Integer.valueOf(capdu.getIndex()), capdu.getApdu(), Arrays.toString(capdu.getSw()));
            byte[] d10 = dVar.d(WearPath.TicPay.APDU, b10);
            k.c("TsmBluetoothCallback", "apdu response %s", b.a(d10));
    ...
    @Override // com.laser.tsm.sdk.tsm.IMobvoiTsmBluetoothInterface
    public void closeSEChannel() {
        b.h().c().g(WearPath.TicPay.CLOSE_SE, new byte[]{1});
        k.c("TsmBluetoothCallback", "closeSEChannel() called , Thread %s", Thread.currentThread());
    }

    @Override // com.laser.tsm.sdk.tsm.IMobvoiTsmBluetoothInterface
    public int connectBluetoothDevice(DeviceInfo deviceInfo) {
        k.a("TsmBluetoothCallback", "connectBluetoothDevice() called with: deviceInfo = [" + deviceInfo.getDeviceName() + "]" + Thread.currentThread());
        return 0;
    }

    @Override // com.laser.tsm.sdk.tsm.IMobvoiTsmBluetoothInterface
    public void disconnectBluetoothDevice() {
        k.a("TsmBluetoothCallback", "disconnectBluetoothDevice");
    }
    ...
    public int getDeviceInfo(DeviceInfo deviceInfo) {
        ...
        k.c("TsmBluetoothCallback", "getDeviceInfo: %s", Arrays.toString(split));
        if (split.length != 5) {
            return 22;
        }
        deviceInfo.setDeviceId(split[0]);
        deviceInfo.setDeviceMac(split[1]);
        boolean a10 = q.a(this.f34410a, WearPath.TicPay.PAY, "bus_card_test_mode", false);
    ...
    public List<Rapdu> transiveAPDU(List<Capdu> list) {
        ...
        for (Capdu capdu : list) {
            arrayList.add(capdu.getApdu());
            arrayList2.add(c.h(capdu.getSw()));
            k.c("TsmBluetoothCallback", "capdu index: %s, apdu %s, exp sw %s", Integer.valueOf(capdu.getIndex()), capdu.getApdu(), Arrays.toString(capdu.getSw()));
        }
        ...
        while (true) {
            ...
            k.c("TsmBluetoothCallback", "capdu index: %s, rapdu apdu %s, exp sw %s", Integer.valueOf(capdu2.getIndex()), str2, str);
            if (!c.m(capdu2.getSw(), str)) {
                k.s("TsmBluetoothCallback", "sw error: capdu index: %s, rapdu sw %s, exp sw %s", Integer.valueOf(capdu2.getIndex()), str, Arrays.toString(capdu2.getSw()));
                break;
            }
            i10 += 2;
            i11++;
